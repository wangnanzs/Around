...
func handlerPost(w http.ResponseWriter, r *http.Request) {
	...
	if p.Type == "image" {
		...
		if score, err := annotate(uri); err != nil {
			...
			p.Face = score
			fmt.Println("faceConfidence", p.Face)
		}
	}
	...
	if err != nil {
		http.Error(w, "Failed to save to Elastic Search", http.StatusInternalServerError)
		return
	}

	fmt.Println(" Post is saved")
}
